    ...
    class SecondLevel
    {
        public static async Task WorkerHttpAsync(HttpListenerContext c)
        {
           ...
           catch (Exception e)
           {
            ...
            var wc = connectionE.GeWrapperConnection().Result;
            try
            {
                var b = await wc.Server.SubscriptionSubscriberCountAsync(Utils.ChanelName(mDataIn));
                if (b == 0)
                {
                ...
                var sub = wc.ConnectionMultiplexer.GetSubscriber();
            ...
            {
                Interlocked.Decrement(ref wc.IsOccupied);
            }
